...
class EmbeddingService:
    ...
    def dump(docs: List[Document]) -> List[Dict]:
        """导出属性"""
        return [
            {"id": doc.id, "metadata": doc.metadata, "page_content": doc.page_content}
            for doc in docs
        ]

    def embedding(
        self, docs: List[Document], intent: str, ids: List[str] = None
    ) -> List[str]:
        """用检索器对document进行嵌入"""
    ...
    def from_img(self, dir_path: str, data: List[Dict[str, str]]) -> List[str]:
        ...
        for item in data:
            ...
            docs.append(
                Document(page_content=item["content"], metadata={"source": fpath})
            )  # 构造Document，填入描述和路径
...
async def search(q: Query):
    return JSONResponse(
        content=embed_svc.query(
            query=q.query, intent=q.intent, search_type=q.search_type
        )
    )
