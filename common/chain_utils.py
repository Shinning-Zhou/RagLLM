...
from .const import Intent, system_prompt, contextualize_q_prompt, rag_intent, chat_intent, emoji_intent, router_prompt
...
def get_normal_chain(llm, p_info):
    ...
    return prompt | {"answer": llm | StrOutputParser(name='answer')}  # 模板 -> 大模型 -> 解析成字符串
...
def rag_history_chain(
    ...
    question_answer_chain = create_stuff_documents_chain(model, qa_prompt)  # 构造参考文献链

    rag_chain = create_retrieval_chain(history_aware_retriever, question_answer_chain)  # 构造检索链

    return label, rag_chain

def wrap_router(chains: Dict[str, Runnable]) -> Callable[[Dict], Runnable]:  # 路由函数，用来识别意图并切换链
    def router(info):
        print(info)
...
def intent_history_chain(
    ...
    router_chain = router_prompt | model | StrOutputParser()  # 路由模板 -> 大模型 -> 解析回答

    candidate_chains = {
        "聊天": get_normal_chain(model, chat_intent.model_dump()),  # 普通聊天链
        "问答": rag_history_chain("问答", model, retriever["问答"], rag_intent),  # RAG问答链，根据给的检索器而不同
        "表情包": rag_history_chain("表情包", model, retriever["表情包"], emoji_intent),  # RAG表情包链
    }
